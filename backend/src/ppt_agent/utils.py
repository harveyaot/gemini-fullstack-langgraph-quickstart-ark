...
def extract_ref_documents(messages: List[Dict[str, Any]], top_n_items=15, top_n_chars=1000) -> str:
    """
...
def parse_json_response(content: str, fallback_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
...
def extract_ppt_outline_from_messages(messages: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Extract PPT outline from tool messages in conversation"""
    for msg in messages:
        if msg.get("role") == "tool" and msg.get("name") == "GenPptOutline":
            try:
